...
- 已接入 FMP EOD 只读适配器、脱敏本地缓存和 Yahoo/FMP 日报交叉校验；真实 Key 仅保存于被 Git 忽略的本机 `.env`。
- 实测当前免费权限可读取 SPY EOD；QQQ、IWM、TLT、GLD 返回 HTTP 402（当前套餐限制）。报告会明确显示该限制，不会伪造或回填 FMP 价格。
- 后续需在 FMP 控制台确认可用端点/套餐，或接入另一独立公开日频源覆盖其余观察标的。
